...
def CopyRecoveryFiles(output_zip=None):
  """Copies recovery specific files to the output zip.
  """
  if output_zip is not None:
    output_zip.write(os.environ['OUT'] + '/uramdisk-recovery.img',
                     'recovery/uramdisk.img')


def CopyBootFiles(output_zip=None):
  """Copies files from boot/ folder of the OUT directory to the output zip.
  """
  boot_path = os.environ['OUT'] + '/boot/'
  for base_name in os.listdir(boot_path):
    if output_zip is not None:
      output_zip.write(boot_path + base_name, 'boot/' + base_name)
...
""" % bcb_dev)
  ...
  script.Mount("/boot")
  script.UnpackPackageDir("boot", "/boot")

  CopyBootFiles(output_zip)

  script.Mount("/recovery")
  script.UnpackPackageDir("boot", "/recovery")
  script.UnpackPackageDir("recovery", "/recovery")

  CopyRecoveryFiles(output_zip)
